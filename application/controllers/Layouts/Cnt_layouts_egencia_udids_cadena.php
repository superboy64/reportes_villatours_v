...
class Cnt_layouts_egencia_udids_cadena extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Layouts/Mod_layouts_egencia_udids_cadena');
	...
	public function get_html_layouts_egencia_udids(){
		...
		$this->load->view('Layouts/view_lay_egencia_udids');
	...
	public function get_layouts_egencia_data_import_sp($parametros,$tipo_funcion){
		  ...
		  $id_us = $this->session->userdata('session_id');
		  $rest = $this->Mod_layouts_egencia_udids_cadena->get_layouts_egencia_data_import_sp($id_us);


		   $param_final['rep'] = $rest;
    ...
    public function exportar_txt_lay_egencia_data_import_sp(){
	    ...
	    header('Content-Disposition: attachment;filename=udids_'.$id_us.'.txt');
	    header("Content-Transfer-Encoding: binary"); 
	    header('Pragma: no-cache'); 
	    header('Expires: 0');
	 		
	    set_time_limit(0); 
	    readfile($_SERVER['DOCUMENT_ROOT'].'\reportes_villatours_v\referencias\archivos\archivos_egencia_lay_data_import_sp\udids_'.$id_us.'.txt');

	    //fin del proceso de 3 archivos para el consecutivo    --se elimina inf del usuario de tabla temporal 

	    
		$this->Mod_layouts_egencia_udids_cadena->delete_layouts_egencia_data_import_sp($id_us);
