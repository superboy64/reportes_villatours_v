...
class Cnt_reportes_gastos_ae extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_gastos_ae');
	...
	public function get_html_rep_gastos_ae(){
		...
		$this->load->view('Reportes/view_rep_gastos_ae');
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_gastos_ae->get_razon_social_id_in($ids_cliente);  //optiene razon social
	...
	public function get_grafica_pasajero(){
       ...
       if($tipo_funcion == "aut"){
		...
		$rest_grafica = $this->Mod_reportes_gastos_ae->get_grafica_pasajero($parametros);
	...
	public function exportar_excel_ae(){
		...
		if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
	       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours/referencias/archivos/Reporte_Gastos_ae_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
