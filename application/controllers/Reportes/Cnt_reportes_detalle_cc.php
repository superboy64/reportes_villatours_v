...
class Cnt_reportes_detalle_cc extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_detalle_cc');
	...
	public function get_html_rep_gastos_ae(){
		...
		$this->load->view('Reportes/view_rep_detalle_cc');
	...
	public function get_pasajero_html(){
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_detalle_cc->get_razon_social_id_in($ids_cliente);  //optiene razon social
		...
		$this->load->view('Reportes/view_rep_detalle_cc_html',$array2);
	...
	public function get_pasajero(){
       ...
       if($tipo_funcion == "aut"){
		...
		$rest_pasajero = $this->Mod_reportes_detalle_cc->get_pasajero($parametros);

		$rest_pasajero_det = $this->Mod_reportes_detalle_cc->get_pasajero_detalle($parametros);
		...
		foreach ($rest_pasajero as $key => $value) {
	      ...
	      $dat['TIPO']  = 'TOTALES';
			...
			foreach ($rest_pasajero_det as $key2 => $value2) {
			  ...
			  if( ltrim(rtrim($value->GVC_ID_CENTRO_COSTO)) == ltrim(rtrim($value2->GVC_ID_CENTRO_COSTO)) ){
	...
	public function exportar_excel_ae(){
        ...
        $spreadsheet->getActiveSheet()
			...
			$activeSheet->setCellValue('A5','CC');
			$activeSheet->setCellValue('B5','DESC CC');
			$activeSheet->setCellValue('C5','PASAJERO');
	        $activeSheet->setCellValue('D5','SERIE');
			$activeSheet->setCellValue('E5','DOCUMENTO');
			$activeSheet->setCellValue('F5','CORPORATIVO');
			$activeSheet->setCellValue('G5','CLIENTE');
			$activeSheet->setCellValue('H5','NOMBRE CLIENTE');
        ...
        foreach ($rest_pasajero->array_nuevo_formato as $valor) {
        	...
        	if($valor->TIPO == 'DESCRIPCION' ){
        		...
        		$activeSheet->setCellValue('E'.$cont,$valor->GVC_DOC_NUMERO);
        		$activeSheet->setCellValue('F'.$cont,$valor->GVC_ID_CORPORATIVO);
        		$activeSheet->setCellValue('G'.$cont,$valor->GVC_ID_CLIENTE);
        		$activeSheet->setCellValue('H'.$cont,$valor->GVC_NOM_CLI);
		...
		$activeSheet->setCellValue('F1','DETALLE CENTRO DE COSTO' )->getStyle('F1')->getFont()->setBold(true)->setSize(25);
		...
		if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
       		...
       		$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/Reporte_Detalle_cc_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
