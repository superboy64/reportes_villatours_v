...
class Cnt_reportes_graficos_rut extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_graficos_rut');
	...
	public function get_html_rep_graficos_rut(){
		...
		$this->load->view('Reportes/view_rep_graficos_rut');
		
	}

	public function get_grafica_rutas_html(){
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_graficos_rut->get_razon_social_id_in($ids_cliente);  //optiene razon social
		...
		$this->load->view('Reportes/formatos/view_formato_html_rep_graficos_rut',$array2);
		

	}


	public function get_grafica_rutas(){
		...
		if($tipo_funcion == "aut"){
        	...
        	$ids_serie = $parametros[1];
	        $ids_cliente = $parametros[2];
	        //$ids_servicio = $parametros[2];  //esta vacio --no se ocupa en este reporte 
		...
		$rest_grafica = $this->Mod_reportes_graficos_rut->get_grafica_rutas($parametros);
		...
		foreach ($rest_grafica as $key => $value) {
			...
			$value->GVC_RUTA =  utf8_encode($value->GVC_RUTA);
	...
	public function exportar_excel_ae(){
		...
		$file = UPLOAD_DIR . 'rep_graficos_rut_' .$id_us. '.png';
		...
		$activeSheet->getStyle('B29:E29')->getFill()
	    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
	    ->getStartColor()->setARGB('1f497d');

	    $spreadsheet->getActiveSheet()->getStyle('B29:E29')
		...
		foreach ($rep as $clave => $valor) {
		   ...
		   //if($valor->BOL_DOM != 0 && $valor->BOL_DOM != ""){
			   ...
			   array_push($array_PART_BOL, $PART_BOL);

			   $activeSheet->setCellValue('B'.$contbd,$GVC_RUTA )->getStyle('AL5')->getFont()->setBold(true);
		...
		$total_PART_BOL_DOM_GEN =  array_sum($array_PART_BOL);
		

		$contbd = $contbd + 1;

		$activeSheet->setCellValue('B29','RUTA' )->getStyle('B29')->getFont()->setBold(true);
		$activeSheet->setCellValue('C29','CANT' )->getStyle('C29')->getFont()->setBold(true);
		$activeSheet->setCellValue('D29','TARIFA/Pesos' )->getStyle('C29')->getFont()->setBold(true);
		$activeSheet->setCellValue('E29','%Part' )->getStyle('D29')->getFont()->setBold(true);
		...
		$spreadsheet->getActiveSheet()->getStyle('A1:H'.($contbd + 3))->applyFromArray($styleArray);
		...
		$drawing->setPath($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/rep_graficos_rut_'.$id_us.'.png');
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $activeSheet->setCellValue('B6','GRAFICOS RUTAS')->getStyle('B6')->getFont()->setBold(true)->setSize(25);
		...
		if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
	       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/Reporte_Graficos_rut_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
