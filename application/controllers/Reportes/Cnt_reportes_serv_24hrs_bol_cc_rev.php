...
class Cnt_reportes_serv_24hrs_bol_cc_rev extends CI_Controller {

	public function __construct()
	{

	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_serv_24hrs_bol_cc_rev');
	...
	public function get_html_rep_pasajeros_servicio(){
		...
		$this->load->view('Reportes/view_rep_serv_24hrs_bol_cc_rev');
	...
	public function get_grafica_pasajero_html(){
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_serv_24hrs_bol_cc_rev->get_razon_social_id_in($ids_cliente);  //optiene razon social
		...
		$this->load->view('Reportes/formatos/view_formato_html_rep_serv_24hrs_bol_cc_rev',$array2);
	...
	public function get_grafica_pasajero(){
       ...
       if($tipo_funcion == "aut"){
		...
		$rest_grafica = $this->Mod_reportes_serv_24hrs_bol_cc_rev->get_grafica_pasajero($parametros);
		$rest_provedores_servicio = $this->Mod_reportes_serv_24hrs_bol_cc_rev->get_provedores_servicio($parametros);
	...
	public function exportar_excel_ae(){
		...
		/*$spreadsheet->getActiveSheet()->mergeCells('B10:E10');*/
		...
		$activeSheet->setCellValue('E10','TARIFA');
	    ...
	    foreach ($rest_grafica as $clave => $valor) {  
	      ...
	      foreach ($arr_bd as $clave2 => $valor2) { 
	        	...
	        	$activeSheet->setCellValue('B'.($cont+10),$AGENT )->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
	        	$activeSheet->setCellValue('C'.($cont+10),'BOL.DOM.')->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
	        	$activeSheet->setCellValue('D'.($cont+10),$total_bol_bd )->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
	        	$activeSheet->setCellValue('E'.($cont+10),$total_bd )->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
		      ...
		      $activeSheet->setCellValue('B'.($cont+10),'')->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
        	  $activeSheet->setCellValue('C'.($cont+10),'BOL.INT')->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
        	  $activeSheet->setCellValue('D'.($cont+10),$total_bol_bi )->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
        	  $activeSheet->setCellValue('E'.($cont+10),$total_bi )->getStyle('B'.($cont+10))->getAlignment()->setHorizontal('right');
        	  $cont++; 
		...
		/********************************************************************************/
	  	...
	  	//$spreadsheet->getActiveSheet()->getStyle('A1:F'.($cont+9))->applyFromArray($styleArray);
        ...
        if($tipo_funcion == "aut"){
		 ...
		 if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
	       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/Reporte_Serv_24hrs_bol_cc_rev_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
