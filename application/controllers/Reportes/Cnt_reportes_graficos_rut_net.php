...
class Cnt_reportes_graficos_rut_net extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_graficos_rut_net');
	...
	public function get_html_rep_graficos_rut(){
		...
		$this->load->view('Reportes/view_rep_graficos_rut_net');
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_graficos_rut_net->get_razon_social_id_in($ids_cliente);  //optiene razon social
	...
	public function get_grafica_rutas(){
		...
		$rest_grafica = $this->Mod_reportes_graficos_rut_net->get_grafica_rutas($parametros);
	...
	public function exportar_excel_ae(){
		...
		$activeSheet->setCellValue('B28','RUTA' )->getStyle('B28')->getFont()->setBold(true);
		...
		if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
	       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/Reporte_Graficos_rut_net_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
