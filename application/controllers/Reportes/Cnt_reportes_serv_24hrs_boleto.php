...
class Cnt_reportes_serv_24hrs_boleto extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_serv_24hrs_boleto');
	...
	public function get_html_rep_pasajeros_servicio(){
		...
		$this->load->view('Reportes/view_rep_serv_24hrs_boleto');
	...
	public function get_grafica_pasajero_html(){
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_serv_24hrs_boleto->get_razon_social_id_in($ids_cliente);  //optiene razon social
		...
		$this->load->view('Reportes/formatos/view_formato_html_rep_serv_24hrs_boleto',$array2);
	...
	public function get_grafica_pasajero(){
       ...
       if($tipo_funcion == "aut"){
		...
		$rest_grafica = $this->Mod_reportes_serv_24hrs_boleto->get_grafica_pasajero($parametros);
		$rest_provedores_servicio = $this->Mod_reportes_serv_24hrs_boleto->get_provedores_servicio($parametros);
		...
		foreach ($rest_provedores_servicio as $key => $value) {
			...
			$value->GVC_NOMBRE_PROVEEDOR =  utf8_encode($value->GVC_NOMBRE_PROVEEDOR);
	...
	public function exportar_excel_ae(){
		...
		$spreadsheet->getActiveSheet()->getStyle('B10:E10')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
		...
		$activeSheet->setCellValue('C10','CVE SERVICIO');
		$activeSheet->setCellValue('D10','NOMBRE PROVEEDOR');
		$activeSheet->setCellValue('E10','# TRANSACCIONES');
		$activeSheet->setCellValue('F10','TARIFA');
	    ...
	    foreach ($rest_grafica as $clave => $valor) {  
	      ...
	      if($valor['TOTAL'] != 0 && $valor['TOTAL'] != ""){
	        ...
	        foreach ($rest_provedores_servicio as $clave2 => $valor2) { 
	          ...
	          $AGENT2 = utf8_encode($valor2['AGENT']);
	          $GVC_ID_SERVICIO2 = utf8_encode($valor2['GVC_ID_SERVICIO']);
	      ...
	      foreach ($arr_bd as $clave2 => $valor2) { 
		        ...
		        $GVC_NOMBRE_PROVEEDOR2 = utf8_encode($valor2['GVC_NOMBRE_PROVEEDOR']);
		        ...
		        if($clave2 == 0){
		        	...
		        	$activeSheet->setCellValue('C'.($cont+10),'BOL.DOM.');
		        	$activeSheet->setCellValue('D'.($cont+10),$GVC_NOMBRE_PROVEEDOR2 );
		        	$activeSheet->setCellValue('E'.($cont+10),$TOTAL_BOL2 );
		        	$activeSheet->setCellValue('F'.($cont+10),$TOTAL2);


		        }else{

		        	$activeSheet->setCellValue('B'.($cont+10),'');
		        	$activeSheet->setCellValue('C'.($cont+10),'');
		        	$activeSheet->setCellValue('D'.($cont+10),$GVC_NOMBRE_PROVEEDOR2 );
		        	$activeSheet->setCellValue('E'.($cont+10),$TOTAL_BOL2 );
		        	$activeSheet->setCellValue('F'.($cont+10),$TOTAL2);
		      ...
		      $activeSheet->setCellValue('B'.($cont+10),'');
        	  $activeSheet->setCellValue('C'.($cont+10),'Total BOL.DOM');
        	  $activeSheet->setCellValue('D'.($cont+10),'');
        	  $activeSheet->setCellValue('E'.($cont+10),$total_bol_bd );
        	  $activeSheet->setCellValue('F'.($cont+10),$total_bd);
		      ...
		      foreach ($arr_bi as $clave2 => $valor2) {  
		        ...
		        if($clave2 == 0){
		          ...
		          if(count($arr_bd) == 0){
		          	...
		          	$activeSheet->setCellValue('B'.($cont+10),$AGENT );
		        	$activeSheet->setCellValue('C'.($cont+10),'BOL.INT.');
		        	$activeSheet->setCellValue('D'.($cont+10),$GVC_NOMBRE_PROVEEDOR2 );
		        	$activeSheet->setCellValue('E'.($cont+10),$TOTAL_BOL2 );
		        	$activeSheet->setCellValue('F'.($cont+10),$TOTAL2);
		      ...
		      $activeSheet->setCellValue('B'.($cont+10),'');
        	  $activeSheet->setCellValue('C'.($cont+10),'Total BOL.INT');
        	  $activeSheet->setCellValue('D'.($cont+10),'');
        	  $activeSheet->setCellValue('E'.($cont+10),$total_bol_bi );
        	  $activeSheet->setCellValue('F'.($cont+10),$total_bi);
        	  ...
        	  $activeSheet->setCellValue('D'.($cont+10),'')->getStyle('D'.($cont+10))->getFont()->setBold(true);
        	  $activeSheet->setCellValue('E'.($cont+10),$TOTAL_BOL )->getStyle('E'.($cont+10))->getFont()->setBold(true);
        	  $activeSheet->setCellValue('F'.($cont+10),$TOTAL)->getStyle('F'.($cont+10))->getFont()->setBold(true);
    	  ...
    	  $activeSheet->setCellValue('C'.($cont+10),'')->getStyle('C'.($cont+10))->getFont()->setBold(true);
    	  $activeSheet->setCellValue('D'.($cont+10),'')->getStyle('D'.($cont+10))->getFont()->setBold(true);
    	  $activeSheet->setCellValue('E'.($cont+10),$TOTAL_BOL_GEN)->getStyle('E'.($cont+10))->getFont()->setBold(true);
    	  $activeSheet->setCellValue('F'.($cont+10),$TOTAL_GEN)->getStyle('F'.($cont+10))->getFont()->setBold(true);

	      $activeSheet->getStyle('B'.($cont+10).':F'.($cont+10).'')->getFill()
		    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
		    ->getStartColor()->setARGB('010101');

		  $spreadsheet->getActiveSheet()->getStyle('B'.($cont+10).':F'.($cont+10).'')
	        ->getFont()->getColor()->setARGB('ffffff');

	      $spreadsheet->getActiveSheet()
			    ->getStyle('F11:F'.($cont+10))
			    ->getNumberFormat()
			    ->setFormatCode(PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_CURRENCY_USD_SIMPLE);

		/********************************************************************************/

	    $activeSheet->getStyle('B10:F10')->getFill()
	    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
	    ->getStartColor()->setARGB('1f497d');
	    
	    $spreadsheet->getActiveSheet()->getStyle('B10:F10')
        ->getFont()->getColor()->setARGB('ffffff');
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $activeSheet->setCellValue('B6','Servicio 24 horas boleto')->getStyle('B6')->getFont()->setBold(true)->setSize(25);
        ...
        if($tipo_funcion == "aut"){
		 ...
		 if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
	       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/Reporte_Serv_24hrs_boleto_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
