...
class Cnt_reportes_gastos_gen_net extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_gastos_gen_net');
	...
	public function get_html_rep_gastos_generales(){
		...
		$this->load->view('Reportes/view_rep_gastos_generales_net');
	...
	public function get_prueba_grafica(){
		...
		$rest_grafica = $this->Mod_reportes_gastos_gen_net->get_prueba_grafica($parametros);
		...
		$array1 = array();
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_gastos_gen_net->get_razon_social_id_in($ids_cliente);  //optiene razon social
        	...
        	if($proceso == 'man'){
        		...
        		$this->load->view('Reportes/formatos/view_formato_html_gastos_gen_net',$array2); //antes prueba_graficas
		...
		$grafica = $this->input->post("grafica");
	     ...
	     $pdf->Write(20, '#TRANSAC');
			...
			foreach ($grafica['grafica'] as $clave => $valor) {
    			...
    			$pdf->Output('F',$_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/graficas_export/Reporte_GG_net_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.pdf');
	...
	public function exportar_excel_usuario(){
		...
		$rest = $this->Mod_reportes_gastos_gen_net->get_reportes_gastos_gen($parametros);
       ...
       if($tipo_funcion == "aut"){
       	...
       	$str_fecha = $fecha1.'_A_'.$fecha2;
       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/Reporte_GG_net_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
       	echo json_encode(1); //cuando es uno si tiene informacion
	...
	public function get_rep_gastos_generales(){
		...
		$col = $this->Mod_reportes_gastos_gen_net->get_columnas($id_plantilla,3);
	...
	public function get_catalogo_reportes(){
		...
		$reportes = $this->Mod_reportes_gastos_gen_net->get_catalogo_reportes($id_perfil);
	...
	public function prueba_mach(){
		...
		$reportes = $this->Mod_reportes_gastos_gen_net->prueba_mach();
		$reportesdup = $this->Mod_reportes_gastos_gen_net->prueba_mach_dup();
	...
	public function get_reportes_not_in(){
		...
		$rest = $this->Mod_reportes_gastos_gen_net->get_reportes_perfil_filtrados($reportes);
