...
class Cnt_reportes_serv_24hrs_cs extends CI_Controller {

	public function __construct()
	{
		
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_serv_24hrs_cs');
	...
	public function get_html_rep_pasajeros_servicio(){
		...
		$this->load->view('Reportes/view_rep_serv_24hrs_cs');
	...
	public function get_grafica_pasajero_html(){
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_serv_24hrs_cs->get_razon_social_id_in($ids_cliente);  //optiene razon social
		...
		$this->load->view('Reportes/formatos/view_formato_html_rep_serv_24hrs_cs',$array2);
	...
	public function get_grafica_pasajero(){
       ...
       if($tipo_funcion == "aut"){
		...
		$rest_grafica = $this->Mod_reportes_serv_24hrs_cs->get_grafica_pasajero($parametros);
		$rest_provedores_servicio = $this->Mod_reportes_serv_24hrs_cs->get_provedores_servicio($parametros);
	...
	public function exportar_excel_ae(){
    	 ...
    	 $TOTAL_BOL_GEN = 0;
			foreach ($rest_grafica as $clave => $valor) {  
		      ...
		      if($valor['TOTAL'] != 0 && $valor['TOTAL'] != ""){
		        ...
		        foreach ($rest_provedores_servicio as $clave2 => $valor2) {  
		          ...
		          if($AGENT2 == $AGENT){
		            ...
		            if($count==1){
			          ...
			          $activeSheet->setCellValue('E'.($cont+10),number_format((float)$TOTAL2));
		              ...
		              $activeSheet->setCellValue('C'.($cont+10),$GVC_ID_SERVICIO2 )->getStyle('C'.($cont+10))->getFont()->setBold(true);
		        	  $activeSheet->setCellValue('D'.($cont+10),$TOTAL_BOL2)->getStyle('D'.($cont+10))->getFont()->setBold(true);
			          $activeSheet->setCellValue('E'.($cont+10),number_format((float)$TOTAL2) )->getStyle('E'.($cont+10))->getFont()->setBold(true);


		            }
		            
		          }//fin validacion agent
			    ...
			    $activeSheet->setCellValue('E'.($cont+10),number_format((float)$TOTAL) )->getStyle('E'.($cont+10))->getFont()->setBold(true);
    	  ...
    	  $activeSheet->setCellValue('C'.($cont+10),'')->getStyle('C'.($cont+10))->getFont()->setBold(true);
    	  $activeSheet->setCellValue('D'.($cont+10),$TOTAL_BOL_GEN)->getStyle('E'.($cont+10))->getFont()->setBold(true);
    	  $activeSheet->setCellValue('E'.($cont+10),$TOTAL_GEN)->getStyle('F'.($cont+10))->getFont()->setBold(true);
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $activeSheet->setCellValue('B6','Servicios 24 hrs (CS)')->getStyle('B6')->getFont()->setBold(true)->setSize(25);
        ...
        if($tipo_funcion == "aut"){
		 ...
		 if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
	       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours_v/referencias/archivos/Reporte_Serv_24hrs_cs_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
