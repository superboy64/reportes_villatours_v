...
class Cnt_reportes_detalle_consumos_precompra extends CI_Controller {
	...
	public function get_html_rep_detalle_consumo_precompra(){
		...
		$this->load->view('Reportes/view_rep_detalle_consumo_precompra');
		
	}

	public function get_rep_detalle_consumo_precompra(){
		...
		$rep = $this->rep_detalle_consumo_precompra($parametros);
	...
	public function rep_detalle_consumo_precompra($parametros){
		  ...
		  $rest = $this->Mod_reportes_detalle_consumo_precompra->get_detalle_consumo_precompra($parametros);
		    ...
		    foreach ($rest as $clave => $valor) {
				...
				$valor["GVC_PRECOMPRA"] = utf8_encode($valor["GVC_PRECOMPRA"]); 

				$GVC_ID_CLIENTE = $valor["GVC_ID_CLIENTE"];
				$GVC_PRECOMPRA = $valor["GVC_PRECOMPRA"];
				
				$ID_PRECOMPRA = $this->Mod_reportes_detalle_consumo_precompra->get_rangos_precompra($GVC_ID_CLIENTE,$GVC_PRECOMPRA);
	...
	public function exportal_excel_detalle_consumo_precompra(){
		...
		$activeSheet->setCellValue('Z5','PRECOMPRA');
		$activeSheet->setCellValue('AA5','GVC ID PRECOMPRA');
		$activeSheet->setCellValue('AB5','NUMERO DE EMPLEADO');
		$activeSheet->setCellValue('AC5','CVE FORMA DE PAGO');
		$activeSheet->setCellValue('AD5','FECHA DE RESERVACION');

		$id_usu = $this->session->userdata('session_id');

		if($id_usu == 42){ //campos que solo pueden ver los de istemas

			$activeSheet->setCellValue('AE5','GVC_TIPO_BOLETO');
			$activeSheet->setCellValue('AF5','GVC_EMD');

		}

		$activeSheet->getStyle('A5:AF5')->getFill()
	    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
	    ->getStartColor()->setARGB('1f497d');

	    $spreadsheet->getActiveSheet()->getStyle('A5:AF5')
		...
		$spreadsheet->getActiveSheet()->getStyle('A1:AF'.(count($rep) + 5))->applyFromArray($styleArray);
		$spreadsheet->getActiveSheet()->getStyle('A1:AF5')->applyFromArray($styleArray);
		...
		$str_corporativo = "";
 		...
 		foreach ($rep as $clave => $valor) {
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_detalle_consumo_precompra->get_razon_social_id_in($ids_cliente);  //optiene razon social
		...
		if($id_usu != 42){ //campos que solo pueden ver los de istemas
			...
			$spreadsheet->getActiveSheet()->removeColumn('AE');
			$spreadsheet->getActiveSheet()->removeColumn('AF');
