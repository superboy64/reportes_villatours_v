...
class Cnt_reportes_pasajero_servicio extends CI_Controller {

	public function __construct()
	{
	      parent::__construct();
	      $this->load->model('Reportes/Mod_reportes_pasajero_servicio');
	...
	public function get_html_rep_pasajeros_servicio(){
		...
		$this->load->view('Reportes/view_rep_pasajero_servicio');
	...
	public function get_grafica_pasajero_html(){
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $ids_cliente = array_filter($ids_cliente, "strlen");
            ...
            if(count($ids_corporativo) > 0 ){
                ...
                $rz = $this->Mod_reportes_pasajero_servicio->get_razon_social_id_in($ids_cliente);  //optiene razon social
		...
		$this->load->view('Reportes/formatos/view_formato_html_rep_pasajero_servicio',$array2);
	...
	public function get_grafica_pasajero(){
       ...
       if($tipo_funcion == "aut"){
		...
		$rest_grafica = $this->Mod_reportes_pasajero_servicio->get_grafica_pasajero($parametros);
		$rest_provedores_servicio = $this->Mod_reportes_pasajero_servicio->get_provedores_servicio($parametros);
	...
	public function exportar_excel_ae(){
        ...
        $activeSheet->setCellValue('B27','PASAJERO / SERVICIO');
		$activeSheet->setCellValue('C27','# TRANSACCIONES');
		$activeSheet->setCellValue('D27','TARIFA/Pesos');
        ...
        foreach ($rest_grafica as $clave => $valor) {  
		      ...
		      if($valor['TOTAL'] != 0 && $valor['TOTAL'] != ""){
		        ...
		        $activeSheet->setCellValue('B'.($cont+27),$NOMBRE_PAX )->getStyle('B'.($cont+27))->getFont()->setBold(true);
		        $activeSheet->setCellValue('C'.($cont+27),$TOTAL_BOL)->getStyle('C'.($cont+27))->getFont()->setBold(true);
			    $activeSheet->setCellValue('D'.($cont+27),number_format((float)$TOTAL) )->getStyle('D'.($cont+27))->getFont()->setBold(true);
		    	...
		    	foreach ($rest_provedores_servicio as $clave2 => $valor2) {  
			       ...
			       $GVC_NOMBRE_PROVEEDOR2 = $valor2['GVC_NOMBRE_PROVEEDOR'];
			        ...
			        if($NOMBRE_PAX2 == $NOMBRE_PAX){
			        	...
			        	$activeSheet->setCellValue('B'.($cont+27),$GVC_NOMBRE_PROVEEDOR2 )->getStyle('B'.($cont+27))->getAlignment()->setHorizontal('right');
				        $activeSheet->setCellValue('C'.($cont+27),$TOTAL_BOL2)->getStyle('B'.($cont+27))->getAlignment()->setHorizontal('right');
					    $activeSheet->setCellValue('D'.($cont+27),number_format((float)$TOTAL2) )->getStyle('B'.($cont+27))->getAlignment()->setHorizontal('right');
	  	...
	  	$spreadsheet->getActiveSheet()->getStyle('A1:F'.($cont+26))->applyFromArray($styleArray);
		...
		$drawing->setPath($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours/referencias/archivos/rep_gastos_ae_'.$id_us.'.png');
		...
		$ids_corporativo = array_filter($ids_corporativo, "strlen");
  	    ...
  	    $activeSheet->setCellValue('B6','Total por pasajero y servicio')->getStyle('B6')->getFont()->setBold(true)->setSize(25);
        ...
        if($tipo_funcion == "aut"){
		 ...
		 if($tipo_funcion == "aut"){
			...
			$str_fecha = $fecha1.'_A_'.$fecha2;
	       	$Excel_writer->save($_SERVER['DOCUMENT_ROOT'].'/reportes_villatours/referencias/archivos/Reporte_Total_Pasajero_servicio_net_'.$str_fecha.'_'.$id_correo_automatico.'_'.$id_reporte.'.xlsx');
