...
class Mod_reportes_graficos_rut_net extends CI_Model {
   ...
   public function get_grafica_rutas($parametros){
    ...
    select
        ...
        total=(convert(real,DETALLE_NC.DET_NC_TAR_MN )+convert(real,(select ISNULL(ROUND(SUM(IMPXSERVNC.CANTIDAD),2),0) from
          ...
          TIPO = 'NC',
          DETALLE_NC.DET_NC_CONCEPTO as GVC_RUTA
    ...
    $select3 = "select top 10 GVC_RUTA=FAC.GVC_RUTA,";
         ...
         if($id_servicio == 'BD'){
               ...
               SUM(
                ...
                from #TEMPFAC AS FAC1 WHERE not FAC1.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO = 'BD') AND FAC1.GVC_ID_SERVICIO = 'BD' AND FAC1.GVC_RUTA = FAC.GVC_RUTA),BOL_INT = 0,";
         ...
         if($id_servicio == 'BI'){
              ...
              from #TEMPFAC AS FAC2 WHERE not FAC2.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO = 'BI') AND FAC2.GVC_ID_SERVICIO = 'BI' AND FAC2.GVC_RUTA = FAC.GVC_RUTA),";

         }

  
    $select3 = $select3 . "TOTAL_BOL=SUM(case when(FAC.total >= 0) then 1 else-1 end) from #TEMPFAC as FAC where not FAC.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO = '".$id_servicio."') and FAC.GVC_ID_SERVICIO = '".$id_servicio."' ";

    $select3 = $select3 . " group by FAC.GVC_RUTA order by TOTAL_BOL desc";
