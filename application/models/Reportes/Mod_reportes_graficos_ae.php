...
class Mod_reportes_graficos_ae extends CI_Model {
   ...
   public function get_grafica_provedor($parametros){
    ...
    $this->db->query("create table #NombreTabla6(
     ...
     GVC_NOMBRE_PROVEDOR varchar(500) null,
    ...
    PROVEEDORES.NOMBRE as GVC_NOMBRE_PROVEDOR,
...
UNION ALL
    ...
    $select2 = "select ID_PROVEDOR=SUP.GVC_ID_PROVEEDOR,DESC_SERVICIO=SUP.GVC_DESC_SERVICIO,NOMBRE_PROVEDOR=SUP.GVC_NOMBRE_PROVEDOR,";
    ...
    SUM(
      ...
      case when(GVC_TARIFA_MON_BASE = '0.00') then 0 else 

        convert(numeric,CASE WHEN GVC_TIPO = 'NC' THEN '-'+ GVC_TARIFA_MON_BASE ELSE GVC_TARIFA_MON_BASE END)
    ...
    from #NombreTabla6 WHERE GVC_ID_SERVICIO = 'BD' AND GVC_ID_PROVEEDOR = SUP.GVC_ID_PROVEEDOR),
    ...
    from #NombreTabla6 WHERE GVC_ID_SERVICIO = 'BI' AND GVC_ID_PROVEEDOR = SUP.GVC_ID_PROVEEDOR),
    ...
    ";
        ...
        foreach ($ids_servicio as $clave => $valor) {  //obtiene clientes asignados
         ...
         if($valor == 'BD'){
          ...
          $select2 = $select2 . "BOL_DOM=(SELECT 
    ...
    from #NombreTabla6 WHERE GVC_ID_SERVICIO = 'BD' AND GVC_ID_PROVEEDOR = SUP.GVC_ID_PROVEEDOR),BOL_INT = 0,";
          
         }else if($valor == 'BI'){
          
          $select2 = $select2 . "BOL_DOM = 0,BOL_INT=(SELECT 
    ...
    $select2 = $select2 . "TOTAL_BOL=SUM(case when(SUP.GVC_TIPO = 'F') then 1 else-1 end) from #NombreTabla6 AS SUP where 1=1 and SUP.GVC_ID_SERVICIO in ('BD','BI') ";

    $select2 = $select2 . " group by SUP.GVC_ID_PROVEEDOR, SUP.GVC_DESC_SERVICIO, SUP.GVC_NOMBRE_PROVEDOR order by SUP.GVC_NOMBRE_PROVEDOR ASC";
...
UNION ALL
    ...
    $select2 = $select2 . "TOTAL_BOL=SUM(case when(SUP.GVC_TIPO = 'F') then 1 else-1 end) from #NombreTabla6 AS SUP where 1=1 and SUP.GVC_ID_SERVICIO = '".$tip_ser."' ";
