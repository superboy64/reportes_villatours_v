...
class Mod_reportes_tabulador_aereolineas extends CI_Model {
   ...
   public function get_grafica_provedor($parametros){
    ...
    GVC_DESC_SERVICIO varchar(500) null, 
    ...
    select 
      TIPO_SERVICIO.DESC_SERVICIO as GVC_DESC_SERVICIO,
      GVC_ID_SERIE=DATOS_FACTURA.ID_SERIE,GVC_ID_SERIE_FAC='',
      ...
      GVC_ID_SERVICIO=PROV_TPO_SERV.ID_SERVICIO,
      ...
      GVC_FECHA_SALIDA=convert(char(12),convert(datetime,CONCECUTIVO_BOLETOS.FECHA_SAL),105),
      GVC_FECHA_REGRESO=convert(char(12),convert(datetime,CONCECUTIVO_BOLETOS.FECHA_REG),105),
      GVC_FECHA_EMISION_BOLETO=convert(char(12),convert(datetime,CONCECUTIVO_BOLETOS.FECHA_EMI),105),
      ...
      GVC_FECHA_RESERVACION=convert(char(12),convert(datetime,GDS_GENERAL.FECHA_RESERVACION),105) from
      DBA.DATOS_FACTURA,
      DBA.DETALLE_FACTURA,
      DBA.CLIENTES,
    ...
    $select1 = $select1." 
    and ".$condicion_fecha." between cast('".$fecha1."' as date) and cast('".$fecha2."' as date) and DETALLE_FACTURA.contra <> 'S' and DETALLE_FACTURA.EMD <> 'S'  
    ";
    ...
    select 
      TIPO_SERVICIO.DESC_SERVICIO as GVC_DESC_SERVICIO,
      GVC_ID_SERIE=NOTAS_CREDITO.ID_SERIE,
    ...
    $select2 = $select2."
      ...
      and ".$condicion_fecha." between cast('".$fecha1."' as date) and cast('".$fecha2."' as date) and DETALLE_NC.det_nc_ctra <> 'S' 
   ...
   if($all_dks == 0){
    ...
    ID_PROVEDOR=FAC.GVC_ID_PROVEEDOR,DESC_SERVICIO=FAC.GVC_DESC_SERVICIO,NOMBRE_PROVEDOR=FAC.GVC_NOMBRE_PROVEEDOR,";

    if(count($ids_servicio) == 0 || count($ids_servicio) > 1){
    ...
    BOL_DOM=(SELECT 
        ...
        from #TEMPFAC AS FAC1 WHERE not FAC1.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO = 'BD') AND FAC1.GVC_ID_SERVICIO = 'BD' AND FAC1.GVC_ID_PROVEEDOR = FAC.GVC_ID_PROVEEDOR),

        BOL_INT=(SELECT 

        SUM(

          case when(FAC2.GVC_TARIFA_MON_BASE = '0.00') then 0 else convert(numeric,FAC2.GVC_TARIFA_MON_BASE) END

        ) 

        from #TEMPFAC AS FAC2 WHERE not FAC2.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO = 'BI') AND FAC2.GVC_ID_SERVICIO = 'BI' AND FAC2.GVC_ID_PROVEEDOR = FAC.GVC_ID_PROVEEDOR),";
        ...
        foreach ($ids_servicio as $clave => $valor) {  //obtiene clientes asignados
             ...
             if($valor == 'BD'){
              ...
              $select3 = $select3 . "BOL_DOM=(SELECT 
                ...
                from #TEMPFAC AS FAC1 WHERE not FAC1.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO = 'BD') AND FAC1.GVC_ID_SERVICIO = 'BD' AND FAC1.GVC_ID_PROVEEDOR = FAC.GVC_ID_PROVEEDOR),BOL_INT = 0,";
              
             }else if($valor == 'BI'){
              
              $select3 = $select3 . "BOL_DOM = 0,BOL_INT=(SELECT 
    ...
    $select3 = $select3 . "TOTAL_BOL=SUM(case when(FAC.GVC_TOTAL >= 0) then 1 else-1 end)";

    $select3 = $select3 . " from #TEMPFAC as FAC where
         not FAC.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO in ('BD','BI'))
     and FAC.GVC_ID_SERVICIO in ('BD','BI')";

    $select3 = $select3 . " group by FAC.GVC_ID_PROVEEDOR,FAC.GVC_DESC_SERVICIO,FAC.GVC_NOMBRE_PROVEEDOR order by FAC.GVC_NOMBRE_PROVEEDOR ASC";
...
//*****************************************
      public function get_datos_grafica($parametros,$tip_ser){
   ...
   if($all_dks == 0){
    ...
    $select3 = $select3 . " from #TEMPFAC as FAC where
         not FAC.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC WHERE GVC_ID_SERVICIO = '".$tip_ser."')
     and FAC.GVC_ID_SERVICIO = '".$tip_ser."' "; 
