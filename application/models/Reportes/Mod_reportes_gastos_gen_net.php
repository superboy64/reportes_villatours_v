...
class Mod_reportes_gastos_gen_net extends CI_Model {
   ...
   if($all_dks == 0){
    ...
    name=case when(FAC.GVC_ID_SERVICIO = 'BOL. DOM.' or FAC.GVC_ID_SERVICIO = 'BOL. INT.' or FAC.GVC_ID_SERVICIO = 'HOTEL INT.' or FAC.GVC_ID_SERVICIO = 'TRANSP.' or FAC.GVC_ID_SERVICIO = 'CARGO X SERV.') then FAC.GVC_ID_SERVICIO
    else 'OTROS2' end,y=SUM(case when(FAC.GVC_TARIFA_MON_BASE = '0.00') then 0 else convert(numeric,FAC.GVC_TARIFA_MON_BASE) end),total_boletos=SUM(case when(FAC.GVC_TOTAL >= 0) then 1 else-1 end) 

    from #TEMPFAC as FAC where
         not FAC.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC)

    group by(case when(FAC.GVC_ID_SERVICIO = 'BOL. DOM.' or FAC.GVC_ID_SERVICIO = 'BOL. INT.' or FAC.GVC_ID_SERVICIO = 'HOTEL INT.' or FAC.GVC_ID_SERVICIO = 'TRANSP.' or FAC.GVC_ID_SERVICIO = 'CARGO X SERV.') then FAC.GVC_ID_SERVICIO
    else 'OTROS2' end) ";
