...
class Mod_reportes_layout_seg extends CI_Model {
  ...
  public function get_columnas($id_plantilla,$id_reporte){
      ...
      if($id_plantilla == 0){
          ...
          $query = $db_prueba->query('SELECT rpc.*,rep_col.nombre_columna_vista,rep_col.order_column FROM rpv_reporte_plantilla_columnas rpc

                                  inner join reportes_villa_tours.rpv_reporte_columnas rep_col on rep_col.id = rpc.id_col where id_plantilla = '.$id_plantilla);
   ...
   public function get_columnas_hide($id_plantilla,$id_reporte){
      ...
      if($id_plantilla == 0){
          ...
          $query = $db_prueba->query('SELECT * FROM rpv_reporte_columnas where id not in ( select id_col from rpv_reporte_plantilla_columnas where id_plantilla = '.$id_plantilla.') and id_rep = '.$id_reporte);
          return $query->result();
      }
   ...
   public function get_reportes_layout_seg($parametros){
    ...
    dba.gds_general.id_serie,fecha_fac=convert(char(12),datos_factura.fecha,105),
    detalle_factura.emd,
    ...
    fecha_salida=convert(char(12),dba.gds_vuelos.fecha_salida,105),
    documento=dba.detalle_factura.fac_numero,
    solicitor=convert(varchar,dba.datos_factura.solicito),
    type_of_service=(case when((TIPO_SERVICIO.id_tipo_servicio <> 'BD' and TIPO_SERVICIO.id_tipo_servicio <> 'BI' and TIPO_SERVICIO.id_tipo_servicio <> 'HOTNAC' and TIPO_SERVICIO.id_tipo_servicio <> 'HOTINT') and((select count(*) from gds_hoteles where consecutivo = dba.gds_general.consecutivo) > 0)) then 'HOTNAC_VARIOS' else convert(varchar,TIPO_SERVICIO.id_tipo_servicio)
    end),supplier=convert(varchar,dba.gds_vuelos.id_la),
    ...
    hora_llegada_vu10='',
    numero_vuelo10='',
    clase_reservada10='',hotel1='',fecha_entrada1='',fecha_salida1='',noches1='',
    ...
    country10=null,Car_class1='',Delivery_Date1='',Nr_days1='',Place_delivery1='',Place_delivery_back1='',Departure_date1='',Car_class2='',Delivery_Date2='',Nr_days2='',Place_delivery2='',Place_delivery_back2='',Departure_date2='',Car_class3='',Delivery_Date3='',Nr_days3='',Place_delivery3='',Place_delivery_back3='',Departure_date3='',Car_class4='',Delivery_Date4='',Nr_days4='',Place_delivery4='',Place_delivery_back4='',Departure_date4='',Car_class5='',Delivery_Date5='',Nr_days5='',Place_delivery5='',Place_delivery_back5='',Departure_date5='',Car_class6='',Delivery_Date6='',Nr_days6='',Place_delivery6='',Place_delivery_back6='',Departure_date6='',Car_class7='',Delivery_Date7='',Nr_days7='',Place_delivery7='',Place_delivery_back7='',Departure_date7='',Car_class8='',Delivery_Date8='',Nr_days8='',Place_delivery8='',Place_delivery_back8='',Departure_date8='',Car_class9='',Delivery_Date9='',Nr_days9='',Place_delivery9='',Place_delivery_back9='',Departure_date9='',Car_class10='',Delivery_Date10='',Nr_days10='',Place_delivery10='',Place_delivery_back10='',Departure_date10='',buy_in_advance=datediff(dd,dba.concecutivo_boletos.fecha_emi,dba.gds_vuelos.fecha_salida),record_localizador=convert(varchar,dba.gds_general.record_localizador) 
    ...
    (dba.detalle_factura.prov_tpo_serv = dba.prov_tpo_serv.prov_tpo_serv) 
    and

    DBA.TIPO_SERVICIO.ID_TIPO_SERVICIO = DBA.PROV_TPO_SERV.ID_SERVICIO 
    and dba.detalle_factura.id_stat = 1 "; 
    
    if($id_intervalo == '5'){

      $condicion_fecha = "cast((cast(datos_factura.fecha as char(10)) +' '+ datos_factura.hora_fac) as datetime)";
    ...
    $select = $select . "
    and ".$condicion_fecha."  between '".$fecha1."' and '".$fecha2."' and
    TIPO_SERVICIO.id_tipo_servicio <> 'CS'
    ";
    ...
    $select2 = " 
    select 
    concecutivo_boletos.conjunto,
      datos_factura.tpo_cambio,
      dba.gds_vuelos.combustible as combustible,
      dba.gds_general.id_serie,
      fecha_fac=convert(char(12),cast(datos_factura.fecha as date),105),      
      detalle_factura.emd,
      GVC_NOM_CLI=convert(varchar,DATOS_FACTURA.CL_NOMBRE),
      GVC_ID_CORPORATIVO=convert(varchar,CLIENTES.ID_CORPORATIVO),
      consecutivo=dba.gds_general.consecutivo,
      ...
      documento=dba.detalle_factura.fac_numero,
      solicitor=convert(varchar,dba.datos_factura.solicito),
      --type_of_service=convert(varchar,TIPO_SERVICIO.id_tipo_servicio),
      type_of_service=(case when((TIPO_SERVICIO.id_tipo_servicio <> 'BD' and TIPO_SERVICIO.id_tipo_servicio <> 'BI' and TIPO_SERVICIO.id_tipo_servicio <> 'HOTNAC' and TIPO_SERVICIO.id_tipo_servicio <> 'HOTINT') and((select count(*) from gds_hoteles where consecutivo = dba.gds_general.consecutivo) > 0)) then 'HOTNAC_VARIOS' else convert(varchar,TIPO_SERVICIO.id_tipo_servicio)
      end),supplier=convert(varchar,dba.gds_vuelos.id_la),
      codigo_producto=convert(varchar,(if dba.detalle_factura.numero_bol is not null then 'AIR' else 'TRA' endif)),
      final_user=convert(varchar,dba.detalle_factura.nom_pasajero),
      ticket_number=convert(varchar,concecutivo_boletos.conjunto),
      typo_of_ticket=convert(varchar,dba.TIPO_SERVICIO.id_alcance_serv),

      fecha_emi=convert(char(12),cast(dba.concecutivo_boletos.fecha_emi as date),105),

      city='',country='',total_emission='',
      --*****millas********
      total_millas='',
      --**fin millas***
      total_Itinerary1='',
      ...
      and (concecutivo_boletos.conjunto <> 0)
     ...
     if($all_dks == 0){
        ...
        if($cont_cliente == 0){  
         ...
         if($cont_suc > 0){
              ...
              $select2 = $select2 . "and DATOS_FACTURA.ID_SUCURSAL in (".$str_suc.")";

         }
         if($cont_serie > 0){  //ya

              $select2 = $select2 . " and DATOS_FACTURA.ID_SERIE in (".$str_ser.") "; //ya
     ...
     $query_rows2 = $this->db->query($select2);

     $result2 = $query_rows2->result();
          
     for($x=0;$x<count($result2);$x++){


      array_push($result,$result2[$x]);
     ...
     return $result;
      ...
      //$query_count = $this->db->query("execute sp_new_Martha_vuelos_count '".$fecha1."','".$fecha2."',$id_cliente,$id_corporativo,$id_usuario");
   ...
   public function get_hoteles_iris($tipo_funcion,$fecha_ini_proceso,$id_intervalo,$fac_numero,$fecha1,$fecha2,$id_serie){
      ...
      $res = $db_prueba->query("SELECT 
                                ...
                                WHERE iris_reserv_serv.FAC_NUMERO = $fac_numero and iris_reserv_serv.id_clave <> 'CXS' and iris_cupones.fecha_cupon between '$fecha1' and '$fecha2' and iris_reserv_serv.id_serie='$id_serie'");
