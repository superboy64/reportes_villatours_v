...
class Mod_reportes_gastos_ae_net extends CI_Model {
   ...
   public function get_grafica_pasajero($parametros){
    ...
    $select3 = "select top 10 NOMBRE_PAX=FAC.GVC_NOM_PAX,";
    ...
    from #TEMPFAC AS FAC1 WHERE not FAC1.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC) AND FAC1.GVC_ID_SERVICIO = 'BD' AND FAC1.GVC_NOM_PAX = FAC.GVC_NOM_PAX),BOL_INT = 0,";
    ...
    from #TEMPFAC AS FAC2 WHERE not FAC2.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC) AND FAC2.GVC_ID_SERVICIO = 'BI' AND FAC2.GVC_NOM_PAX = FAC.GVC_NOM_PAX),";
    ...
    $select3 = $select3 . "TOTAL_BOL=SUM(case when(FAC.total >= 0) then 1 else-1 end) from #TEMPFAC as FAC where not FAC.GVC_DOC_NUMERO = any(select distinct GVC_FAC_NUMERO from #TEMPNC) and FAC.GVC_ID_SERVICIO = '".$id_servicio."' ";

    $select3 = $select3 . " group by FAC.GVC_NOM_PAX order by TOTAL_BOL desc";
